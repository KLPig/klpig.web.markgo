...
class Actions(Instructions):
    def __init__(self, cmd: str):
        ...
        try:
            ...
            raise errors.MarkGoArgumentNumberError(3, len(c))
...
class VariablesSet(Instructions):
    def __init__(self, cmd: str):
        ...
        try:
            ...
            self.val = ' '.join(c[2:])
            if self.op != '=':
                raise errors.MarkGoSyntaxError(c[1])
        except IndexError:
            raise errors.MarkGoArgumentNumberError(3, len(c))
...
class Functions(Instructions):
    def __init__(self, cmd: str):
        ...
        try:
            ...
            raise errors.MarkGoArgumentNumberError(2, len(c))
...
def modularize(code: str) -> list[Instructions]:
    ...
    for c in cmd:
        if c[0] == '!':
            ...
            raise errors.MarkGoSyntaxError(c[0])
    return cmds


file = open('target.mgtgt').read() + '.mgins'
